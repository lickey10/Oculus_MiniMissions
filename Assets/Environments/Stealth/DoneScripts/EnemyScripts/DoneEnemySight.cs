...
public class DoneEnemySight : MonoBehaviour
{
	...
	void OnTriggerStay (Collider other)
    {
        ...
        if(other.transform.root.gameObject == player)
        {
            ...
            if (Physics.Linecast(transform.position, player.transform.position, out hit) &&
                            Vector3.Angle(hit.collider.transform.position - transform.position, transform.forward) <= fieldOfViewAngle * 0.5f &&
                             hit.collider.transform.gameObject == player)
            {
                //return true;
                playerInSight = true;
   ...
   //                     //playerInSight = true;
			...
			int playerLayerZeroStateHash = playerAnim?playerAnim.GetCurrentAnimatorStateInfo(0).nameHash:0;
			int playerLayerOneStateHash = playerAnim?playerAnim.GetCurrentAnimatorStateInfo(1).nameHash:0;
